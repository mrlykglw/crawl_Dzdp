import requests
import re
url='https://www.dianping.com/ajax/json/shopDynamic/basicHideInfo?shopId=k7a1VkZePZmh0ZxC'
...
#print(res.json()['msg']['shopInfo']['phoneNo'])
result=res.json()['msg']['shopInfo']['phoneNo']
phone_number=result.replace('<d class="num">','').replace(';</d>','').replace('**','')

address=res.json()['msg']['shopInfo']['address'].replace('<e class="address">','').replace(';</e>','').replace('<d class="num">','').replace(';</d>','')
